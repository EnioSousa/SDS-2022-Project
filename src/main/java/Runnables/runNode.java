    ...
    public static void main(String[] args) throws InterruptedException, NoSuchAlgorithmException {
        ...
        while (i != 0) {
            if (i == 1) {
                ...
                System.out.println("Enter product id, and bid");
                byte[] prodId = scan.next().getBytes(StandardCharsets.UTF_8);
                int bid = scan.nextInt();

                Transaction transaction =
                        new Transaction(node.getNodeInfo().getId(),
                                Bootstrap.bootstrapId, prodId, bid);
